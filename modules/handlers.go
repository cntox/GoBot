...
func handleQuizBotMessage(client *telegram.Client, m *telegram.NewMessage) error {
	...
	log.Printf("DEBUG correctOption bytes=%v q=%q", correctOption, cleanedQuestion)

	// Use SendPoll with the correct answer as index.
	// SendPoll internally assigns Option=[]byte{byte(i)} to each answer.
	// correctOption from QuizBot results is the ORIGINAL option bytes (e.g. [49] = ASCII '1').
	// We need to find which position (index) in cleanedAnswers matches correctOption.
	correctIdx := -1
	for i, ans := range cleanedAnswers {
		ansObj := ans.(*telegram.PollAnswerObj)
		if bytes.Equal(ansObj.Option, correctOption) {
			correctIdx = i
			break
		}
	}
	if correctIdx < 0 {
		log.Printf("correctOption %v not found in answers", correctOption)
		client.SendMessage(localTarget, "Error: correct answer not found")
		return nil
	}
	log.Printf("DEBUG correctIdx=%d", correctIdx)

	// SendPoll assigns options as Option=[]byte{0},[]byte{1},...
	// CorrectAnswers = [correctIdx] (zero-based index, gogram handles encoding)
	sentMsg, err := client.SendPoll(localTarget, cleanedQuestion,
		cleanedOptionsFromAnswers(cleanedAnswers),
		&telegram.PollOptions{
			IsQuiz:         true,
			CorrectAnswers: []int{correctIdx - 1},
			PublicVoters:   false,
		})
	if err != nil {
	...
	// Close the poll — rebuild with same index-based options
	closedAnswers := make([]telegram.PollAnswer, len(cleanedAnswers))
	for i, ans := range cleanedAnswers {
		ansObj := ans.(*telegram.PollAnswerObj)
		closedAnswers[i] = &telegram.PollAnswerObj{
			Text:   ansObj.Text,
			Option: []byte{byte(i)}, // match what SendPoll assigned
		}
	}
	closedPoll := &telegram.InputMediaPoll{
		...
		CorrectAnswers: []int32{int32(correctIdx - 1)},
	}
