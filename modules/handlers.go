		...
		for _, btn := range row.Buttons {
			if cb, ok := btn.(*telegram.KeyboardButtonCallback); ok {
				if strings.Contains(strings.ToLower(string(cb.Text)), textSearch) {
					m.Click(&telegram.ClickOptions{Data: cb.Data})
					return true
				}
			}
		}
...
func handleQuizBotMessage(client *telegram.Client, m *telegram.NewMessage) error {
	...
	for _, ans := range originalPoll.Answers {
		...
		optionBytes = append(optionBytes, ansObj.Option)
	}

	// Find correct answer index (zero-based) for PollOptions.CorrectAnswers
	correctIdx := -1
	for i, ob := range optionBytes {
		if string(ob) == string(correctOption) {
			correctIdx = i
			break
		}
	}
	if correctIdx < 0 {
		log.Println("Correct option not matched by index")
	...
	// Build closed InputMediaPoll directly to close the poll via EditMessage
					...
					for _, btn := range row.Buttons {
						if cb, ok := btn.(*telegram.KeyboardButtonCallback); ok {
							if strings.Contains(strings.ToLower(string(cb.Text)), "try again") {
								msg.Click(&telegram.ClickOptions{Data: cb.Data})
