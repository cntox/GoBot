...
func handleQuizBotMessage(client *telegram.Client, m *telegram.NewMessage) error {
	...
	// Keep original Option bytes intact — required for CorrectAnswers int32 encoding.
	// Formula (from gogram helpers.go): option = (option<<8) | int32(b) for each byte.
	...
	// Encode correctOption -> int32 using gogram's own formula
	var correctAnswerInt int32
	for _, b := range correctOption {
		correctAnswerInt = (correctAnswerInt << 8) | int32(b)
	}
	log.Printf("DEBUG correctOption=%v -> int32=%d q=%q", correctOption, correctAnswerInt, cleanedQuestion)

	// Build open poll InputMediaPoll directly (bypass SendPoll which mangles Option bytes)
	openPoll := &telegram.InputMediaPoll{
		Poll: &telegram.Poll{
			ID:       originalPoll.ID,
		...
		CorrectAnswers: []int32{correctAnswerInt},
	}

	targetPeer, err := client.ResolvePeer(localTarget)
	if err != nil {
		log.Printf("Failed to resolve target peer: %v", err)
		return err
	}

	sendResp, err := client.MessagesSendMedia(&telegram.MessagesSendMediaParams{
		Peer:     targetPeer,
		Media:    openPoll,
		RandomID: rand.Int63(),
		Message:  "",
	...
	// Extract sent message ID from the Updates response
	var sentMsgID int32
	var updateList []telegram.Update
	switch u := sendResp.(type) {
	...
	for _, upd := range updateList {
		if mu, ok := upd.(*telegram.UpdateMessageID); ok {
			sentMsgID = mu.ID
			break
		}
		if nm, ok := upd.(*telegram.UpdateNewMessage); ok {
			if msg, ok := nm.Message.(*telegram.MessageObj); ok {
				sentMsgID = msg.ID
				break
			}
		}
	...
	if sentMsgID > 0 {
		...
		if _, err := client.EditMessage(localTarget, sentMsgID, closedPoll); err != nil {
			log.Printf("Failed to close poll: %v", err)
		} else {
			log.Println("Poll closed successfully")
		}
	} else {
		log.Println("Warning: sentMsgID=0, poll not closed")
	}
