...
func handleQuizBotMessage(client *telegram.Client, m *telegram.NewMessage) error {
	...
	// correctOption is ASCII byte e.g. [48]='0', [49]='1', [50]='2'...
	// Convert to zero-based index: '0'->0, '1'->1, '2'->2
	correctIdx, _ := strconv.Atoi(string(correctOption))
	if correctIdx < 0 {
		correctIdx = 0
	}
	log.Infof("FINAL correctIdx=%d correctOption=%v", correctIdx, correctOption)
