...
func handleQuizBotMessage(client *telegram.Client, m *telegram.NewMessage) error {
	...
	// Build cleaned options and find correct answer index.
	// QuizBot original answers have arbitrary Option bytes (e.g. 0x41, 0x42...).
	// SendPoll re-assigns options as []byte{0}, []byte{1}, []byte{2}... (simple index bytes).
	// So we match by position: correctIdx = position of answer whose original bytes == correctOption.
	var cleanedOptions []string
	correctIdx := -1
	for i, ans := range originalPoll.Answers {
		ansObj, ok := ans.(*telegram.PollAnswerObj)
		if !ok {
			continue
		}
		cleanedOptions = append(cleanedOptions, cleanAnswerText(ansObj.Text.Text))
		// Match: compare each byte of original option with correctOption
		if len(ansObj.Option) > 0 && len(correctOption) > 0 && ansObj.Option[0] == correctOption[0] {
			correctIdx = i
		} else if fmt.Sprintf("%v", ansObj.Option) == fmt.Sprintf("%v", correctOption) {
			correctIdx = i
		}
	}

	log.Printf("DEBUG correctOption=%v correctIdx=%d options=%v", correctOption, correctIdx, cleanedOptions)

	if correctIdx < 0 || correctIdx >= len(cleanedOptions) {
		log.Printf("Correct option bytes %v not matched among %d answers", correctOption, len(originalPoll.Answers))
		client.SendMessage(localTarget, "Error: could not match correct answer index")
		return nil
	}

	// Send the quiz poll. SendPoll assigns answers as Option=[]byte{0},[]byte{1},...
	// CorrectAnswers is the zero-based index of the correct option.
	sentMsg, err := client.SendPoll(localTarget, cleanedQuestion, cleanedOptions, &telegram.PollOptions{
		IsQuiz:         true,
	...
	// Close the poll via EditMessage with InputMediaPoll{Closed:true}.
	// Must use same Option bytes as SendPoll: []byte{byte(i)}.
	var closedAnswers []telegram.PollAnswer
	for i, opt := range cleanedOptions {
		closedAnswers = append(closedAnswers, &telegram.PollAnswerObj{
			Text:   &telegram.TextWithEntities{Text: opt},
			Option: []byte{byte(i)},
		})
	}
	...
	closedPoll := &telegram.InputMediaPoll{
		...
		CorrectAnswers: []int32{int32(correctIdx)},
	}
