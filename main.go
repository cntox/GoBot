...
// PollSession stores poll creation data per (chat, user)
type PollSession struct {
	InstructionMsgID int32
	Question         string
	Options          []string
	Ready            bool
}
...
func handleStart(m *telegram.NewMessage, client *telegram.Client) error {
	...
	sessions.Store(key, &PollSession{
		InstructionMsgID: sentMsg.ID, // int32
		Ready:            false,
	})
	return nil
}
...
func handlePollDataReply(m *telegram.NewMessage, client *telegram.Client) error {
	...
	// m.ReplyToMsgID is a method that returns int32
...
func handleCallback(cb *telegram.CallbackQuery, client *telegram.Client) error {
	// cb.Data is []byte, convert to string
	if string(cb.Data) != "ready_start" {
		return nil
	}

	// Access message and sender via cb.Msg and cb.Sender
	msg := cb.Msg
	chatID := msg.Chat.ID
	userID := cb.Sender.ID
	key := fmt.Sprintf("%d:%d", chatID, userID)
	...
	if !ok || !val.(*PollSession).Ready {
		_, _ = cb.Answer("Please send your poll data first (reply to the instruction message).", true)
		return nil
	}
	session := val.(*PollSession)

	// Answer callback to close the loading indicator
	_, _ = cb.Answer("Starting countdown...", false)
